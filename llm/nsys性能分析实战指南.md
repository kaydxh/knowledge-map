...
| 发现的问题 | 根因分析 | 优化方案 | 预期效果 |
|-----------|---------|---------|----------|
| 100-200ms GPU 空闲间隙 | vLLM 调度器等待新请求 / prefill→decode 切换开销 | 增大并发请求量，让 scheduler 有更多 sequence 可调度 | 消除 GPU 气泡，吐量 +20-30% |
| Memory 操作仅占 1.1% | 数据传输不是瓶颈 | 无需优化 | - |
| 通信 stream 无活动（0.2%） | 单卡 TP=1，无多卡通信开销 | 如需扩展可考虑 TP=2/4 | 线性扩展吐量 |
...
| 发现的问题 | 根因分析 | 优化方案 | 预期效果 |
|-----------|---------|---------|----------|
| 主引擎线程串行执行所有任务 | vLLM 单线程调度架构 | 关注 Python GIL，升级 vLLM 版本 | 减少调度间隙 |
| 首次 cudaEventSynchronize 65.885ms | CUDA 上下文初始化 + Graph 预录制 | 正常现象，可用 warmup 请求提前触发 | 避免影响线上首请求 |
| Profiler Overhead 引入 2-5% 时间膨胀 | nsys 采集机制开销 | 分析时忽略绿色块，关注相对比例 | 不影响定性结论 |
...
| 发现的问题 | 根因分析 | 优化方案 | 预期效果 |
|-----------|---------|---------|----------|
| GEMM 占 GPU 56% 时间，全部是 BF16 | 未启用量化，Tensor Core 没有充分利用 | 启用 INT8/AWQ 量化 | GEMM 耗时降 50%，整体提速 ~1.5x |
| splitkv_kernel 单次 614µs，占 11.5% | max-model-len 可能配置过大（如 38400） | 减小 max-model-len 到实际所需 | splitkv 耗时降 ~4.7x |
| 大量小 kernel 调用 3万+次 | 框架层碎片化的辅助计算 | CUDA Graph 已覆盖 62%，升级 vLLM 可进一步优化 | 减少 launch overhead |
...
| 发现的问题 | 根因分析 | 优化方案 | 预期效果 |
|-----------|---------|---------|----------|
| CPU 88.5% 时间在等待 GPU | GPU 密集计算场景的典型特征，不是异常 | 无需优化，这说明 CPU 不是瓶颈 | - |
| cudaMemcpyAsync 被调用 12.5 万次 | 每次 step 都需要传输 input 数据 | 批量打包小数据 memcpy | 减少 API 调用开销 |
| vLLM 启动时一次性预分配显存 | 良好的设计模式，避免运行时动态分配 | 无需优化 | - |
...
| 发现的问题 | 根因分析 | 优化方案 | 预期效果 |
|-----------|---------|---------|----------|
| 99.5% kernel 都在排队 | GPU 满载运行，这是健康状态 | 无需优化 | - |
| 极端情况 TMax 达 226-448ms | prefill 大请求导致前面 kernel 排队很久 | 启用 chunked prefill 将长请求拆分 | 减少极端延迟 |
| 单线程提交所有 kernel（TID 592） | vLLM 单线程架构 | 当前未成为瓶颈，无需优化 | - |
...
| 发现的问题 | 根因分析 | 优化方案 | 预期效果 |
|-----------|---------|---------|----------|
| 大量 4B 小 memcpy，带宽仅 1-3 MiB/s | 框架逐个传输标量参数 | 批量打包小数据为一次 memcpy | 减少 API 调用次数 |
| 47.81 MiB 传输使用 Pageable 内存（16.44 GiB/s） | Host buffer 未使用 Pinned 分配 | 改用 cudaMallocHost 分配 | 带宽从 16 → 25 GiB/s |
| 大量小 kernel（Grid=1）SM 利用率极低 | 操作无法简单并行化 | 依赖 CUDA Graph 消除 launch overhead | 已通过 Graph 缓解 |
...
| 发现的问题 | 根因分析 | 优化方案 | 预期效果 |
|-----------|---------|---------|----------|
| Softmax/LogSoftmax 仅用 1-2 个 SM（108 个中的 0.9-1.9%） | Softmax 需要对整个 vocabulary 归一化，难以简单并行 | 使用 FlashInfer/Triton 自定义 Softmax | SM 利用率提升数十倍 |
| computeDigitCumSum 调用 2万+次，Grid=1 | CUB 前缀和操作的固有限制 | CUDA Graph 已覆盖，消除 launch overhead | 减少调度开销 |
| GEMM 并行度优秀（2374 block） | cuBLAS 自动选择了合适的 tile size | 无需优化 | - |

---

## 8. CUDA Graph 原理与分析

> 💡 **CUDA Graph 通俗理解**：就像把一道菜的做法（切菜→炒菜→装盘）提前录制成视频，每次做菜只需要按播放键，不用再一步步喝叫厨师。传统方式是每次都喝一声“切菜！”“炒菜！”“装盘！”，喝的时间（launch 开销）比实际做菜（kernel 执行）还多。
...
| 发现的问题 | 根因分析 | 优化方案 | 预期效果 |
|-----------|---------|---------|----------|
| 37% GPU 时间是 eager launch | prefill 阶段无法用 Graph（变长输入） | 将 prefill 占比降低（减小 max-model-len） | 减少 eager launch 占比 |
| 100+ Graph 预录制占用额外显存 | vLLM 为每个 batch size 预录制 Graph | 可通过 gpu-memory-utilization 控制 | 平衡显存和性能 |
| CUDA Graph 已覆盖 62% GPU 时间 | decode 阶段全部走 Graph 路径 | 保持开启，不要用 --enforce-eager | launch 开销减少 ~100x |

---

## 9. 端到端实战案例串讲

下面以本次实际分析为例，完整还原从"发现问题"到"找到根因"的全流程，帮你建立"什么问题 → 啥原因导致的 → 怎么解决"的分析思路。

### Step 1：发现问题

**现象**：vLLM 推理服务吞吐不达预期，per-token 延迟偏高。

**环境**：NVIDIA A100-SXM4-40GB，单卡 TP=1，BF16 精度。

### Step 2：采集 nsys 数据

```bash
nsys profile --trace=cuda,nvtx,osrt --cuda-memory-usage=true -o vllm_report <vllm_command>
```

### Step 3：先看时间线 → 发现 GPU 气泡

打开报告，在 Timeline View 上立即发现 **100-200ms 的 GPU 空闲间隙**。

→ **初步结论**：GPU "饿着"了，调度器没及时喊活给它做。

### Step 4：看 Kernel Summary → 发现 GEMM 占 56%、无量化

GEMM 全部是 `ampere_bf16_s16816gemm` 系列，没有 FP8/INT8 kernel。

→ **结论**：Tensor Core 没有充分利用，启用量化可以直接砂半。

### Step 5：看 Grid/Block Summary → 发现 Softmax 只用 1 个 SM

`LogSoftMaxForwardEpilogue` Grid=1,1,1，调用 5060 次，每次 108 个 SM 只有 1 个在干活。

→ **结论**：Softmax kernel 并行度极差，但占比不高（~3%），优先级中等。

### Step 6：看 GPU Trace → 发现小 memcpy 和 Pageable 内存

大量 4B 小 memcpy（带宽仅 1-3 MiB/s），以及 47.81 MiB Pageable 传输（16.44 vs 25 GiB/s）。

→ **结论**：数据传输效率有提升空间，但总占比仅 1.1%，不是主要矛盾。
...
1️⃣ GEMM 未量化（占 GPU 56%）      → 启用 INT8/AWQ，预计提速 ~1.5x
2️⃣ GPU 气泡 100-200ms            → 增大并发/batch，吞吐 +20-30%
3️⃣ splitkv 慎时 614µs（占 11.5%） → 减少 max-model-len，加速 ~4.7x
4️⃣ Softmax SM 利用率 <2%        → 升级 vLLM 或用 FlashInfer
...
**问题**：当前全部使用 BF16 GEMM（占 GPU 56% 时间），没有启用任何量化。
...
**为什么这是最高优先级**：GEMM 占了 GPU 56% 的时间，量化直接减半这部分耗时，收益最大。

#### 10.1.2 减少 max-model-len

**问题**：`flash_fwd_splitkv_kernel`（decode attention）单次执行 **614µs**，是所有 kernel 中最慢的，占 GPU 时间 11.5%。它的耗时与 KV cache 长度成正比。
...
**问题**：时间线上有 **100-200ms 的 GPU 空闲间隙**，GPU 没活干。
...
**为什么重要**：
- CUDA API Summary 显示 CPU **88.5% 时间在等 GPU**（`cudaEventSynchronize` + `cudaStreamSynchronize`），说明 CPU 不是瓶颈
- Kernel Launch & Exec Time 显示 99.5% 的 kernel 都在排队，GPU 一直在忙
- 但时间线上仍有 **100-200ms 的空闲间隙** → 说明瓶颈在**请求到达率或调度间隔**上
- GEMM 在更大 batch 下可以更好地利用 Tensor Core（Grid 数量线性增长）

### 10.2 🟡 中优先级（有明确改善空间）

#### 10.2.1 替换低并行度的 Softmax/LogSoftmax Kernel

**问题**：Grid/Block Summary 暴露了严重问题：

| Kernel | Grid | Block 数 | SM 利用率 | 调用次数 |
|--------|------|---------|-----------|---------|
| `LogSoftMaxForwardEpilogue` | 1,1,1 | **1** | **0.9%** | 5060 |
| `SoftMaxForward` | 2,1,1 | **2** | **1.9%** | 2876 |
| `computeDigitCumSum` | 1,1,1 | **1** | **0.9%** | 20240 |

A100 有 108 个 SM，这些 kernel 只用了 1-2 个，**其余 106-107 个 SM 全部空闲**。
...
**问题**：GPU Trace 显示大量 **4B 的小 memcpy**，throughput 仅 1-3 MiB/s：

```
4B HtoD memcpy: 3.711µs, throughput = 1.03 MiB/s   ← 效率极低
4B HtoD memcpy: 1.152µs, throughput = 3.31 MiB/s   ← 效率极低
```

而 `cudaMemcpyAsync` 被调用了 **124,797 次**（占 CPU 时间 2.2%）。
...
**问题**：GPU Trace 显示某些大块传输使用了 **Pageable 内存**：

```
47.81 MiB: Pageable → Device, 16.44 GiB/s
96.28 KiB: Pageable → Device, 9.90 GiB/s
```

而 Pinned 内存的传输带宽可达 **~25 GiB/s**（PCIe Gen4 x16 理论值）。
...
当前 62% GPU 时间被 CUDA Graph 覆盖，效果显著。确保不要使用 `--enforce-eager`。

#### 10.3.5 分析那 37% 未被 CUDA Graph 捕获的 Kernel
