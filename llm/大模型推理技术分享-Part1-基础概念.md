...
**核心特点**：使用**双向注意力**——每个 token 能关注序列中**所有位置**的 token（既能看左边的全部，也能看右边的全部，不受方向限制），因此擅长**理解**而非生成。

> ⚠️ **注意**：这里说"左右两边"不是指只看紧邻的 token，而是指**左边方向的全部 + 右边方向的全部 = 整个序列**。强调"左右"是为了与 GPT（Decoder-Only）的"只能看左边"做对比，突出**方向**上的区别，而非距离限制。Self-Attention 天生就是全局连接的。
...
每个 token 的 K 和 V 是通过**线性投影矩阵**从 token 嵌入向量计算而来的（Q/K/V 的概念和 Attention 公式已在 [§0.4 自注意力机制](#①-自注意力机制self-attention) 中介绍，此处聚焦**数值计算示例**）。
...
上面 Step 2 中 K₁、V₁ 的计算与 Step 1 **完全相同**（因为 W_k、W_v 是固定权重，x₁ 也没变），但朴素实现还是重新算了一遍。随着序列变长，这种重复计算呈 O(n²) 增长——这正是 KV Cache 要解决的问题。

> **关键结论**：K 和 V 的本质就是 `token嵌入向量 × 权重矩阵` 的矩阵乘法结果。W_k、W_v 是模型训练时学到的参数（推理时固定不变），所以同一个 token 在同一层的 K、V 值永远不变，缓存后可以直接复用。
>
> Attention 三步计算的原理（点积打分 → softmax 归一化 → 加权聚合）和 K/V 分离的设计动机已在 [§0.4 自注意力机制](#①-自注意力机制self-attention) 中详细说明，此处不再重复。
...
> - 当你看到 `max_model_len=4096` 参数时，它直接决定了每个请求的 KV Cache 大小，也就决定了并发数上限
> - Llama-2-7B 每个请求 KV Cache 约 1GB，A100-80GB 理论并发上限约 80 个（还要减去模型权重占用）

### 1.6 KV Cache 显存是什么时候分配的？

一个常见问题：当你配置 `gpu_memory_utilization=0.9` 时，KV Cache 的显存是在模型加载后就占用了，还是推理时才分配？

**答案是分"预留"和"填充"两步**：

```
GPU 总显存（如 A100-80GB）
├── ① 模型权重（加载后立即占用，如 7B FP16 ≈ 14GB）
├── ② 临时计算开销（激活值等）
└── ③ 剩余部分 = KV Cache 可用池
      计算方式: (GPU总显存 × gpu_memory_utilization) - 模型权重 - 其他开销
```

| 时机 | 发生了什么 | 显存状态 |
|------|-----------|---------|
| **模型加载时** | 加载权重到 GPU | 权重占用显存 |
| **vLLM 引擎初始化时** | 计算剩余显存，预分配 KV Cache 物理块池 | **显存已被 CUDA malloc 预留**，但块内没有有效数据 |
| **Prefill 阶段** | 批量计算输入的 K/V，写入物理块 | 块被标记为已用，**数据被实际填充** |
| **Decode 阶段** | 每步追加新 token 的 K/V | 逐步占用更多块 |
| **请求结束** | 释放该请求的所有块 | 块回到空闲池，可被新请求复用 |

> 💡 **关键点**：`gpu_memory_utilization=0.9` 控制的是 vLLM **启动时就预留的 KV Cache 显存池大小**。这块显存在引擎初始化完成后就已经被 CUDA 分配了（`nvidia-smi` 会立刻看到显存占用），但 KV Cache 的**数据填充**是在 Prefill 和 Decode 阶段动态进行的。类似操作系统一开始就划好了内存页帧池，但页帧里有没有数据取决于进程是否在用。
...
**详解：∂Loss/∂W（梯度）是什么，怎么算？**

∂Loss/∂W 的含义是：**如果微调一下权重 W，Loss（损失）会怎么变？** 它告诉优化器每个参数该往哪个方向调、调多大步，最终让模型的预测越来越准。

∂Loss/∂W 是一个**与 W 形状相同的矩阵**，每个元素的含义：
- ∂Loss/∂W[i][j] = **+3.5**：增大 W[i][j] 会让 Loss 变大 → 应该**减小**这个参数
- ∂Loss/∂W[i][j] = **-2.0**：增大 W[i][j] 会让 Loss 变小 → 应该**增大**这个参数
- ∂Loss/∂W[i][j] = **≈ 0**：这个参数对 Loss 影响不大 → 几乎不用动

梯度既给出了**方向**（正/负），也给出了**幅度**（绝对值大小），这就是梯度下降（Gradient Descent）的依据。

**核心数学原理——链式法则（Chain Rule）**：

以单个线性层 `Y = X @ W` 为例，假设后续层已经算好了 dL/dY（Loss 对 Y 的梯度，即"上游梯度"），则：
```
                  链式法则
∂Loss     ∂Loss     ∂Y
───── = ─────── × ─────
...
前向：Y = X @ W = [2×0.5+3×0.3, 2×0.1+3×0.8] = [1.9, 2.6]
```
假设反向传播回来到这一层时，上游梯度为 `dL/dY = [0.4, -0.7]`，则：
```
∂Loss/∂W = Xᵀ @ dL/dY

         ┌   ┐                  ┌                 ┐
       = │ 2 │ @ [0.4, -0.7] = │ 2×0.4   2×(-0.7)│ = [[ 0.8, -1.4],
         │ 3 │                  │ 3×0.4   3×(-0.7)│    [ 1.2, -2.1]]
         └   ┘                  └                 ┘
```

| 参数位置 | 当前值 | 梯度 | 含义 | 更新（lr=0.01） |
|---------|-------|------|------|----------------|
| W[0][0] | 0.5 | +0.8 | 增大→Loss变大 | 0.5 - 0.01×0.8 = 0.492 |
| W[0][1] | 0.1 | -1.4 | 增大→Loss变小 | 0.1 - 0.01×(-1.4) = 0.114 |
| W[1][0] | 0.3 | +1.2 | 增大→Loss变大 | 0.3 - 0.01×1.2 = 0.288 |
| W[1][1] | 0.8 | -2.1 | 增大→Loss变小 | 0.8 - 0.01×(-2.1) = 0.821 |

参数更新公式：`W_new = W_old - lr × ∂Loss/∂W`
...
∂L/∂W₃ = H₂ᵀ @ (∂L/∂ŷ)              ← 最后一层，直接算
∂L/∂W₂ = H₁ᵀ @ (∂L/∂H₂)            ← ∂L/∂H₂ 由 W₃ 传回来
∂L/∂W₁ = Xᵀ  @ (∂L/∂H₁)            ← ∂L/∂H₁ 由 W₂ 传回来

其中"传回来"的过程：
∂L/∂H₂ = (∂L/∂ŷ) @ W₃ᵀ             ← 这就是下文的 dL/dX
∂L/∂H₁ = (∂L/∂H₂) @ W₂ᵀ            ← 一环扣一环，所以叫"链式"法则
```

每一层需要计算两组梯度：**dL/dW**（更新本层参数）和 **dL/dX**（传给前一层继续链式传播），这也是后向传播计算量为前向 2 倍的原因（详见下文）。
...
| 9 | "`gpu_memory_utilization` 只在推理时才占显存" | ❌ vLLM 启动时就通过 CUDA malloc 预留了 KV Cache 的物理块池（`nvidia-smi` 会立刻看到显存占用），但块内数据在 Prefill/Decode 时才填充 |
| 10 | "双向注意力只关注左右相邻的 token" | ❌ "左右两边"是指左边方向的**全部** + 右边方向的**全部** = 整个序列。Self-Attention 是全局连接的，强调"左右"是与 GPT 的"只看左边"对比方向，而非距离 |
...
> 📖 各概念的详细回顾请查阅对应章节末尾的 **"✅ 看完这一章，你至少要记住"** 和上方的 **🎯 速查卡片**。以下是全文的核心脉络串联：

```
Transformer 架构（§0）
  → 自注意力产生 K/V，需要缓存 → KV Cache（§1）
    → KV Cache 的计算发生在两阶段 → Prefill vs Decode（§2）
      → Attention 的 HBM 读写是瓶颈 → FlashAttention（§3）
      → 多请求如何高效调度 → Continuous Batching（§4）
      → 单卡放不下怎么办 → 并行策略 TP/PP/DP（§5）
      → Decode 太慢怎么办 → 推测解码（§6）
```

**一句话串联**：Transformer 的自注意力机制产生了 KV Cache（§1），推理分为 Prefill（计算密集）和 Decode（内存密集）两阶段（§2），FlashAttention 通过分块计算减少显存读写（§3），Continuous Batching 动态调度提升 GPU 利用率（§4），并行策略解决大模型多卡部署（§5），推测解码用小模型加速 Decode（§6）。
