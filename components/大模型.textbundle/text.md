...
- 定义：数据从输入层 → 隐藏层 → 输出层的**单向计算过程**

- 核心目的
	- 训练时：计算预测结果，与真实标签对比得到损失值（Loss）
	- 推理时：直接得到模型输出

- Transformer前向传播流程（以"我爱北京"预测下一个词为例）
	```
	输入："我爱北京" → [token_1, token_2, token_3, token_4]
	
	┌─────────────────────────────────────────────────────────────┐
	│ Step 1: Embedding层                                          │
	│   tokens → 查表 → X (4×d) 维向量矩阵                          │
	│   + Position Embedding（位置信息）                            │
	└─────────────────────────────────────────────────────────────┘
	                              ↓
	┌─────────────────────────────────────────────────────────────┐
	│ Step 2: Transformer Layer × L层（重复L次）                   │
	│                                                              │
	│   2.1 Layer Norm                                             │
	│       X_norm = LayerNorm(X)                                  │
	│                                                              │
	...
	│       X = X + Dropout(FFN(X_norm))  ← 残差连接               │
	│                                                              │
	└─────────────────────────────────────────────────────────────┘
	                              ↓
	┌─────────────────────────────────────────────────────────────┐
	│ Step 3: 输出层                                               │
	│   logits = X[-1] × W_vocab  (最后一个位置 × 词表矩阵)         │
	│   probs = softmax(logits)   → [0.01, 0.02, ..., 0.15, ...]  │
	│                                      ↑                       │
	│                               vocab_size个概率值              │
	└─────────────────────────────────────────────────────────────┘
	                              ↓
	输出：概率最高的token → "天" （预测"我爱北京天"）
	```
...
- 前向传播的核心计算
	...
	|------|----------|--------|
	| Embedding | 查表 | O(n×d) |
	| Attention | 矩阵乘法 QK^T, Attn×V | O(n²×d) |
	| FFN | 两次矩阵乘法 | O(n×d×4d) |
	| Output | 矩阵乘法 × 词表 | O(d×V) |

- 激活值（Activation）
	- 定义：前向传播过程中每层的**中间计算结果**
	- 为什么要保存？反向传播计算梯度时需要用到
	- 显存占用：训练时的主要显存消耗来源

#### 3.1.2 反向传播（Backward Propagation）

- 定义：从输出层 → 隐藏层 → 输入层，**逐层计算梯度**的过程

- 核心目的：计算每个参数对Loss的影响（梯度），用于更新参数

- 计算流程
	```
	Step 1: 计算Loss
	  Loss = CrossEntropy(probs, label)
	  若真实下一个词是"天"，Loss = -log(P("天"))
	
	Step 2: 链式法则求梯度（从后往前）
	  ∂Loss/∂W_vocab → ∂Loss/∂W_FFN → ∂Loss/∂W_Attn → ... → ∂Loss/∂W_Embed
...
- 为什么叫"反向"？
	- 前向：Input → Layer1 → Layer2 → ... → Output
	- 反向：Output → ... → Layer2 → Layer1 → Input（梯度传播方向）

#### 3.1.3 训练 vs 推理的前向传播对比
...
| Dropout | 启用（随机丢弃） | 关闭 |
| 显存占用 | 高（16Φ） | 低（2Φ） |
| 计算模式 | 所有token并行 | Prefill并行 + Decode串行 |
...
- **Decode单步完整计算过程**（以生成"不"为例）

	- 背景：已完成Prefill和Decode Step1，当前输入token为"京"
	```
	已有KV Cache: [北京, 的, 首都, 是, 哪里, ？, 北] （7个token的K、V）
	当前输入: "京"
	```

	- **Step 1: 计算当前token的Q、K、V**
	```
	q_京 = Embed("京") × W_Q   ← 只有1个Q向量
	k_京 = Embed("京") × W_K   ← 新的K向量
	v_京 = Embed("京") × W_V   ← 新的V向量
	```

	- **Step 2: 从KV Cache读取历史K、V**
	```
	K_history = [k_北京, k_的, k_首都, k_是, k_哪里, k_？, k_北]
	V_history = [v_北京, v_的, v_首都, v_是, v_哪里, v_？, v_北]
	            └───────────── 7个历史K、V ─────────────┘
	```

	- **Step 3: 拼接形成完整K、V序列**
	```
	K_all = [K_history, k_京] = [k_北京, k_的, k_首都, k_是, k_哪里, k_？, k_北, k_京]
	V_all = [V_history, v_京] = [v_北京, v_的, v_首都, v_是, v_哪里, v_？, v_北, v_京]
	        └───────────────────── 8个K、V ─────────────────────┘
	```

	- **Step 4: 计算Attention（核心步骤）**
	```
	Score = q_京 × K_all^T / √d_k    ← 1个Q与8个K计算相似度
	
	注意力分数示例：
	        [北京, 的, 首都, 是, 哪里, ？, 北, 京]
	Score = [0.5, 0.2, 1.0, 0.3, 0.5, 0.2, 3.3, 4.0]
	                                         ↑    ↑
	                                   "北"和"京"分数最高（语义相关）
	
	Attention_weights = softmax(Score)
	        = [0.05, 0.02, 0.10, 0.03, 0.05, 0.02, 0.33, 0.40]
	           └─────────────────── 和为1 ───────────────────┘
	```

	- **Step 5: 加权融合得到上下文向量**
	```
	Context = Attention_weights × V_all
	        = 0.05×v_北京 + 0.02×v_的 + 0.10×v_首都 + 0.03×v_是 
	        + 0.05×v_哪里 + 0.02×v_？ + 0.33×v_北 + 0.40×v_京
	        
	含义：融合了所有历史信息，"北"和"京"的信息占比最大
	```

	- **Step 6: 通过FFN和输出层**
	```
	hidden = FFN(Context)                    ← 非线性变换
	logits = hidden × W_vocab                ← 映射到词表空间
	probs = softmax(logits)                  ← 得到每个词的概率
	
	probs = [..., P("不")=0.65, P("是")=0.15, P("市")=0.08, ...]
	                  ↑
	          模型判断"不"概率最高
	```

	- **Step 7: 采样输出 + 更新KV Cache**
	```
	输出token = argmax(probs) = "不"
	KV Cache更新: 追加 k_京 和 v_京
	KV Cache数量: 7 → 8
	```
...
- **关键理解：为什么"京"能输出"不"？**
	...
	| 误解 | 正确理解 |
	|------|----------|
	| 只看当前输入"京" | 通过Attention看到**完整上下文** |
	| 输入→输出的简单映射 | 输入 + KV Cache（历史）→ 输出 |
	| KV Cache只是存储 | KV Cache**参与Attention计算** |

	- 实际决策依据："北京的首都是哪里？北京" → 模型理解问题有误 → 输出"不"
